...
func main() {
  b, err := os.ReadFile("src/2024/day5.test")
  if err != nil {
    log.Fatal(err)
  }
  input := string(b)
  ...
  for _, update := range updates {
    if !inCorrectOrder(update, before, after) {
      update := correctOrder(update, before)
      ans += getMid(update)
    } 
  }
...
func correctOrder(list []int, before map[int][]int) []int {
  corrected := make([]int, len(list))
  for _, li := range list {
    idx := 0
    for _, v := range list {
      if li != v && slices.Contains(before[li], v) {
        idx += 1
      }
    }
    corrected[idx] = li
