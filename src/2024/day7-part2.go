...
func dfs(numbers []int, target, i, currSum int) bool {
	...
	return dfs(numbers, target, i+1, currSum+numbers[i+1]) || dfs(numbers, target, i+1, currSum*numbers[i+1]) || dfs(numbers, target, i+1, concat(currSum, numbers[i+1]))
}

func concat(x, y int) int {
	concatenated := strconv.Itoa(x) + strconv.Itoa(y)
	res, err := strconv.Atoi(concatenated)
	if err != nil {
		log.Fatalf("error %s", err)
	}
	return res
